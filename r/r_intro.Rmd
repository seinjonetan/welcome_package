...
```{r}
# Your code here
mtx <- matrix(6:1, nrow = 2, ncol = 3, byrow = TRUE)
print(mtx)
```
...
```{r}
# Your code here
print_pyramid <- function(levels) {
    if (levels <= 0) {
        cat("Invalid input: levels must be a positive integer\n")
        return
    }
    for (i in 1:levels) {
        cat(rep(" ", levels - i), rep("*", 2 * i - 1), "\n")
    }
}
...
3. The folowwing function is supposed to return the cumulative sum of numbers from 1 to n. However, there is a bug in the code. Find and fix the bug.
...
```{r}
# Your code here
check_sum <- function(credit_card_number) {
    if (nchar(credit_card_number) != 16) {
        cat("Invalid input: credit card number must be 16 digits\n")
        return(FALSE)
    }
    sum <- 0
    for (i in 1:16) {
        digit <- as.integer(substr(credit_card_number, i, i))
        sum <- sum + digit
    }
    if (sum %% 10 == 0) {
        return(TRUE)
...
print(check_sum("1234567890123456"))
```
